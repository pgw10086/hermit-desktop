...
SQLite + trigram FTS5 repository、受限 IPC、快捷键和快速取回浮层。完整 History 直接运行在
真实 DSH Product Surface 中，复用 DSH 公开布局、菜单、输入、按钮、提示、设置行和图标，
样式使用 CSS Modules 与 DSH 语义 token；生产组件只读取 Core 注入的公开 API，不内置 mock
数据或第二套页面壳。快捷取回由桌面 BrowserWindow 加载实际 React renderer 制品，不再使用
...
- 快速取回采用可配置动作映射：Enter、Mod+Enter、Shift+Enter 三个固定组合分别映射
  到使用当前项、只复制、纯文本使用三个动作；默认映射保持 Enter 使用、Mod+Enter 只
  复制、Shift+Enter 纯文本使用，用户可在插件设置中调整。
...
2. **取回速度优先。** 最重要的体验是“唤起 -> 输入 -> 选择 -> 使用”，管理功能不能
   拖慢这条路径。
3. **复制是可靠底座。** 选中历史项后先写回系统剪贴板；自动粘贴只是可降级增强。
...
- 入口统一使用 Core Quick Panel、Tray 和 Shortcut Registry，不创建第二套桌面入口；
- 使用 macOS、Windows、X11/Wayland 的真实平台能力实现捕获和写回；
- 自动粘贴受权限、焦点、UIPI、Wayland 或远程桌面限制时，明确降级为“已复制，请
  手工粘贴”；
...
- 不是第二个桌面壳；不自建 Tray、全局快捷键、菜单栏或 Quick Panel。
...
直接验收桌面 BrowserWindow 加载的实际 renderer 制品。下方已确认图片只作为决策依据和
历史上下文保留，不能据此另建一套页面壳。
...
自动粘贴失败时，内容仍已写入系统剪贴板，浮层显示“已复制，请手工粘贴”。Escape 关闭
且不执行；唤起浮层的全局快捷键由用户在 Core 统一快捷键设置中确认，不能由插件自行
注册，浮层内部三个固定组合的动作映射由插件设置负责。
...
- 预览默认关闭，不占主列表宽度。当前选中项稳定约 400ms 后，在**同一个 BrowserWindow**
  内按需展开约 256px 的贴靠 sidecar；优先向右，空间不足向左，主列表宽度和鼠标锚点
  保持不变。sidecar 打开后在本次浮层会话内保持打开，只随选中项更新。
- 不为预览创建第二个 BrowserWindow、第二个 renderer 或第二套选中状态；Escape、失焦和
...
- 底部只保留会随动作映射更新的 `Enter`、`Mod+Enter`、`Shift+Enter` 提示和“完整历史 ›”；
  ...
  不额外撑高窗口。自动粘贴失败先关闭浮层，再由桌面反馈“已复制，请手工粘贴”。
...
| 按键 | 默认动作 |
| --- | --- |
| Enter | 使用当前项：写回系统剪贴板并 best-effort 自动粘贴 |
| Mod+Enter | 只复制，不尝试自动粘贴；macOS 使用 Command，Windows/Linux 使用 Ctrl |
| Shift+Enter | 以纯文本使用，仅对 TEXT 可用；IMAGE、FILE_LIST 明确不可用 |
| Escape | 关闭浮层，不执行任何内容动作 |

自动粘贴失败不回滚系统剪贴板，关闭浮层并在返回原应用后提示“已复制，请手工粘贴”。
插件设置提供三行固定组合的动作选择器，用户可以为 Enter、Mod+Enter、Shift+Enter
分别选择使用当前项、只复制或纯文本使用。三个动作必须一一对应，重复映射不能保存并
就地说明冲突；系统不自动交换、不静默改写用户选择。纯文本使用在 IMAGE、FILE_LIST
结果上明确不可用，不降级为普通使用。浮层底部的按键提示在设置保存后同步更新。
...
在右侧贴靠显示类型预览，执行后回到原应用；自动粘贴失败保留 copy-only 结果，底部可以
...
| 使用当前项 | 同时写回纯文本和受支持的格式化表示，由目标应用选择可接受格式 |
| 纯文本使用 | 只写回纯文本，明确移除格式 |

来源没有格式化表示时，“使用当前项”和“纯文本使用”的内容结果相同。
...
| 使用当前项/只复制 | 将完整 Canonical 图片写回系统剪贴板 |
| 纯文本使用 | 明确不可用，不静默切换为其他动作 |
...
| 使用当前项/只复制 | 先检查全部引用；全部存在才按原顺序写回系统剪贴板 |
| 失效处理 | 任一引用失效则整组不可用，不覆盖当前剪贴板，并明确标出失效项目 |
| 纯文本使用 | 明确不可用，不静默切换为路径文本 |
...
- Auto-paste unavailable：内容已复制，提示用户手工粘贴。
...
- 写回先生成随机 operation token，以一次 `writeObjects` 写入 Canonical 表示和 token；最终
  generation 或 token 不一致就拒绝继续自动粘贴。
- 自动粘贴分为“请求恢复原目标应用”和“确认目标仍在前台后发送 Command+V”两步；发送前
...
- native 单元测试只使用唯一命名 pasteboard；成品无人值守资格只验证 addon 加载，不碰用户
  General Pasteboard。真实写回、自动粘贴、权限提示和物理快捷键仍属于 disposable 人工资格。

## 里程碑方向

1. **M1 Maccy 核心闭环**：本地捕获、历史、快捷键入口、搜索取回、复制/粘贴、置顶、
   删除、暂停和基础三类内容。
...
- macOS 真实链路覆盖捕获、去重、持久化、重启恢复、搜索、写回、自动粘贴和 copy-only
