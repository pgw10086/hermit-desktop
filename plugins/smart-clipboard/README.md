...
Core 负责 SQLite、本机剪贴板读写、快捷键 Registry、显式粘贴和导出文件，DSH Client 负责
...
动作映射（默认 Enter/左键复制，Option/Alt+左键或 Mod+Enter 显式粘贴）；SQLite + trigram
FTS5 重启恢复；macOS Objective-C++ N-API bridge 的稳定快照、格式化写回和 generation/token
守卫的 best-effort 显式粘贴。Hermit bundled DSH 已通过受控、精确
锁定的 Product Surface source patch 提供 Core-owned 产品入口组和完整 History；同一插件 artifact 在
...
浮层由 Desktop Surface Manager 加载实际 React renderer 制品，默认约 480px 单列；选中项稳定后在同一
Surface 实例内按需展开贴靠 sidecar 预览，共享插件公开的 typed client API，不再使用内联
data URL 页面。插件没有使用私有 Router、DOM 注入或第二套 DSH 页面壳。

快捷键由 Desktop Core 的 `ShortcutRegistry` 统一注册、持久化和释放。请在 DSH 的“设置 ->
快捷键”中修改“打开剪贴板快速取回”。与其他应用或系统冲突时，快捷键中心明确显示冲突，
History 和后台捕获仍保持可用；本插件业务设置页不重复维护全局快捷键。
...
- 显式粘贴失败时保持已复制状态并显示手工粘贴提示；普通复制成功后直接关闭浮层；右键只打开已有预览；
