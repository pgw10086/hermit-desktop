...
import { parseDesktopSurfaceRequest } from '../lib/desktop/desktop-surface-ipc.js'
...
test('Desktop Surface IPC 只接受有限的生命周期操作和语义化选项', () => {
  ...
  assert.deepEqual(parseDesktopSurfaceRequest({
    ...
    options: { anchor: 'cursor', placement: 'adjacent', preferredSize: { width: 720, height: 620 }, topmost: true, session: { type: 'existing', sessionId: 'session-1' } },
  }), {
    op: 'toggle',
    id: 'conversation.quick',
    options: { anchor: 'cursor', placement: 'adjacent', preferredSize: { width: 720, height: 620 }, topmost: true, session: { type: 'existing', sessionId: 'session-1' } },
  ...
  assert.throws(() => parseDesktopSurfaceRequest({ op: 'open', id: 'x', options: { topmost: 'yes' } }), /topmost 无效/u)
  assert.throws(() => parseDesktopSurfaceRequest({ op: 'open', id: 'x', options: { preferredSize: { width: 100, height: 200 } } }), /size 超出范围/u)
